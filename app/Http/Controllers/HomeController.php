...
use App\Models\{ Product, Key, User_Subscription, Pricing_Table, Page, Post, Setting, Newsletter_Subscriber, Reaction, Search, 
  ...
  Product_Price, License, Payment_Link, User_Prepaid_Credit, Custom_Route, Affiliate_Earning, Cashout, Coupon, Temp_Direct_Url };
    ...
    public function checkout_form(Request $request)
    {
      ...
      { 
        foreach($form_config as $name => $props)
        {
          ...
          {
            $form_fields[] = <<<FIELD
              <input type="{$props['type']}" name="{$name}" value="{$props['value']}" class="{$props['class']}">
            FIELD;
          }
            ...
            if($props['label'])
            {
              ...
              $form_fields[] = <<<FIELD
                <div class="field {$props['class']}">
                  <label>{$label}</label>
                  <input type="{$props['type']}" name="{$name}" value="{$props['value']}">
                </div>
              FIELD;
            }
            else
