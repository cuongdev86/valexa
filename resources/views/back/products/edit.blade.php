...
@section('title', \Str::limit($product->name, 50))
...
<div id="product" vhidden>
	<form class="ui large main form" method="post" autocomplete="off" enctype="multipart/form-data" action="{{ route('products.update', $product->id) }}">
		<div class="field">
		...
		<div class="ui one column grid">
			<div class="column tabs">
			  ...
			  <div class="ui tab segment active" data-tab="overview">
			  	<div class="field">
						<label>{{ __('Notify buyers about the update') }}</label>
						<div class="ui selection floating dropdown">
							<input type="hidden" name="notify_buyers" value="{{ old('notify_buyers', '0') }}">
							<div class="text">...</div>
							<div class="menu">
			  	...
			  	<div class="field">
					...
					<div class="field">
						...
						<div class="ui labeled input">
						  ...
						  <input type="text" name="permalink" {{ !config('app.permalink_url_identifer') ? "disabled" : "" }} placeholder="..." value="{{ old('permalink', $product->permalink) }}">
						</div>
					</div>

					<div class="field">
						<label>{{ __('Affiliate link') }}</label>
						<input type="text" name="affiliate_link" placeholder="..." value="{{ old('affiliate_link', $product->affiliate_link) }}">
					</div>
					...
					<div class="field">
						...
						<div class="ui multiple selection floating dropdown" id="subcategories">
							<input type="hidden" name="subcategories" value="{{ old('subcategories', $product->subcategories) }}">
							<i class="dropdown icon"></i>
					...
					<div class="field">
						...
						<textarea name="overview" class="html-editor">{!! old('overview', $product->overview) !!}</textarea>
					</div>

					<div class="field">
						<label>{{ __('Included files') }}</label>
						<input type="text" name="included_files" value="{{ old('included_files', $product->included_files ?? null) }}" placeholder="...">
					</div>

					<div class="field">
						<label>{{ __('Version') }}</label>
						<input type="text" name="version" value="{{ old('version', $product->version ?? null) }}" placeholder="...">
					</div>
					...
					<div class="field">
						...
						<input type="date" name="release_date" value="{{ old('release_date', $product->release_date ?? null) }}" placeholder="...">
					</div>
					...
					<div class="field">
						...
						<input type="date" name="last_update" value="{{ old('last_update', $product->last_update ?? null) }}" placeholder="...">
					</div>
					
					<div class="field">
						<label>{{ __('Tags') }} <sup>({{ __('Optional') }})</sup></label>
						<input type="text" name="tags" value="{{ old('tags', $product->tags ?? null) }}" placeholder="...">
					</div>

					<div class="field">
						<label>{{ __('Preview link') }} <sup>({{ __('Optional') }})</sup></label>
						<input type="text" name="preview_url" class="d-block" placeholder="https://..." value="{{ old('preview_url', $product->preview_url ?? null) }}">
					</div>
					...
					<div class="field">
						...
						<input type="number" name="stock" value="{{ old('stock', $product->stock ?? null) }}">
					...
					<div class="field">
						...
						<div class="ui floating selection dropdown">
							<input type="hidden" name="enable_license" value="{{ old('enable_license', $product->enable_license ?? '0') }}">
							<div class="text">...</div>
							<div class="menu">
								<a class="item" data-value="1">{{ __('Yes') }}</a>
								<a class="item" data-value="0">{{ __('No') }}</a>
					...
					<div class="field">
						...
						<input type="number" name="fake_sales" value="{{ old('fake_sales', $product->fake_sales ?? null) }}">
					</div>

					<div class="field">
						<label>{{ __('Available via subscription only') }}</label>
						<div class="ui floating selection dropdown">
							<input type="hidden" name="for_subscriptions" value="{{ old('for_subscriptions', $product->for_subscriptions ?? '0') }}">
							<div class="text">...</div>
							<div class="menu">
								<a class="item" data-value="1">{{ __('Yes') }}</a>
								<a class="item" data-value="0">{{ __('No') }}</a>
					...
					<div class="field">
						...
						<div class="ui floating search selection dropdown countries">
							<input type="hidden" name="country_city[country]" value="{{ old('country_city.country', $product->country_city->country ?? null) }}">
							<div class="text">...</div>
							<div class="menu">
					...
					<div class="field">
						...
						<div class="ui floating search selection dropdown cities">
							<input type="hidden" name="country_city[city]" value="{{ old('country_city.city', $product->country_city->city ?? null) }}">
							<div class="text">...</div>
							<div class="menu"></div>
						</div>
					</div>
							...
							<div class="fluid card">
								...
								<div class="content flex-direction-column">
									...
									<div class="w-100">
										...
										<input type="url" name="main_file_download_link" placeholder="{{ __('Download link') }}" value="{{ old('main_file_download_link', $product->direct_download_link) }}" @change="setDefaultDrive" :class="{disabled: inputIsOff('download')}" class="mt-1 {{ old('main_file_download_link', $product->direct_download_link) ? 'active' : '' }}">									
									</div>
								</div>
							</div>
							...
							<div class="fluid card">
								...
								<div class="content">
									<div class="w-100" v-if="oldUploads.screenshots.length">
										@if($screenshots)
											<div class="ui fluid pink large label">
												{{ $screenshots }}
												<i class="close icon ml-auto mr-0" @click="deleteExistingFile('{{ "public/storage/screenshots/{$screenshots}" }}', 'screenshots')"></i>
											</div>						
										@else
											<div class="screenshots w-100">
												@foreach($screenshots_files as $screenshot_file)
												<div class="image position-relative" style="background-image: url('{{ asset_("storage/screenshots/{$screenshot_file}") }}')">
													<i class="close circular icon ml-auto link mr-0" @click="deleteScreenshot($event, '{{ "public/storage/screenshots/{$screenshot_file}" }}')"></i>
												</div>
												@endforeach
											</div>
										@endif
									...
									<div class="w-100" v-else>
										...
										<button class="ui basic large circular blue fluid button" :class="{disabled: inputIsOff('screenshots')}" type="button" @click="selectFile('screenshots')">{{ __('Browse') }}</button>
									</div>
								</div>
							</div>
									...
									<div class="w-100 mt-1">
										...
										<div class="ui left labeled input preview-type fluid">
										  <div class="ui dropdown basic files button label">
										  	<input type="hidden" name="preview_type" value="{{ old('preview_type', $product->preview_type ?? 'other') }}">
										  ...
										  <input type="url" name="preview_direct_link" :class="{disabled: inputIsOff('preview')}" placeholder="{{ __('Direct link') }}" value="{{ old('preview_direct_link', $product->preview_direct_link) }}" class="{{ old('preview_direct_link', $product->preview_direct_link) ? 'active' : '' }}">
			  ...
			  <div class="ui tab segment" data-tab="hidden-content">
					<textarea name="hidden_content" class="html-editor">{{ old('hidden_content', $product->hidden_content ?? null) }}</textarea>
			  </div>
			  	...
			  	<div class="table wrapper licenses">
				  	<table class="ui basic table unstackable w-100">
					  	...
					  	<tr>
					  		...
					  		<td>
					  			<input type="number" name="license[price][{{ $license->id }}]" step="0.01" placeholder="{{ __('Default price') }}" value="{{ old("license.price[$license->id]", $product_prices[$license->id]['price'] ?? null) }}">
					  		</td>
					  		<td>
					  			<input type="number" name="license[promo_price][{{ $license->id }}]" step="0.01" placeholder="{{ __('Promo price') }}" value="{{ old("license.promo_price[$license->id]", $product_prices[$license->id]['promo_price'] ?? null) }}">
					  		</td>
					  	</tr>
					  	@endforeach
					  </table>
			  	...
			  	<div class="table wrapper free">
				  	<table class="ui basic table unstackable w-100">
					  	<tr>
					  		...
					  		<td><input type="number" step="0.0001" name="minimum_price" value="{{ old('minimum_price', $product->minimum_price ?? null) }}"></td>
					  	</tr>
					  </table>
				  </div>
			  	...
			  	<div class="table wrapper free">
				  	<table class="ui basic table unstackable w-100">
					  	<tr>
					  		...
					  		<td><input type="datetime-local" name="free[from]" value="{{ format_date(old('free.from', $product->free->from ?? null), 'Y-m-d\TH:i') }}" placeholder="{{ __('From') }}"></td>
					  		<td><input type="datetime-local" name="free[to]" value="{{ format_date(old('free.from', $product->free->to ?? null), 'Y-m-d\TH:i') }}" placeholder="{{ __('To') }}"></td>
					  	</tr>
					  </table>
				  </div>
				  ...
				  <div class="table wrapper promo_price">
				  	<table class="ui basic table unstackable w-100">
					  	<tr>
					  		...
					  		<td><input type="datetime-local" name="promotional_price_time[from]" value="{{ format_date(old('promotional_price_time.from', $product->promotional_price_time->from ?? null), 'Y-m-d\TH:i') }}" placeholder="{{ __('From') }}"></td>
					  		<td><input type="datetime-local" name="promotional_price_time[to]" value="{{ format_date(old('promotional_price_time.to', $product->promotional_price_time->to ?? null), 'Y-m-d\TH:i') }}" placeholder="{{ __('To') }}"></td>
					  	</tr>
					  </table>
				  </div>
			  </div>
			  ...
			  <div class="ui tab segment" data-tab="table-of-contents">
			    <table class="ui celled unstackable single line table" 
						...
						<tbody>
							@if(old('text_type', $product->text_type ?? null))
							
							@foreach(old('text_type', $product->text_type ?? null) as $key => $text_type)
							<tr>
								<td>
								...
								<td class="ten column wide right aligned">
									<input type="text" name="text[{{ $key }}]" placeholder="..." value="{{ old('text', $product->text ?? null)[$key] ?? '' }}" class="toc-text">
								</td>
			  ...
			  <div class="ui tab segment" data-tab="faq" data-dict='{"Question": "{{ __('Question') }}", "Answer": "{{ __('Answer') }}", "Remove": "{{ __('Remove') }}", "Add": "{{ __('Add') }}"}'>
						@if(old('question', $product->question ?? null) && old('answer', $product->answer ?? null))

							@foreach(old('question', $product->question ?? []) ?? [] as $k => $qa)
								<div class="ui segment">
									...
									<div class="field">
										...
										<textarea name="answer[{{ $k }}]" cols="30" rows="3" class="faq-answer" placeholder="...">{{ old('answer')[$k] ?? $product->answer[$k] ?? '' }}</textarea>
									</div>
				...
				<div class="ui tab segment" data-tab="additional-fields" data-dict='{"Name": "{{ __('Name') }}", "Value": "{{ __('Value') }}", "Remove": "{{ __('Remove') }}", "Add": "{{ __('Add') }}"}'>
					<div class="field">
						...
						<input type="text" name="label" value="{{ old('label', $product->label ?? null) }}">
					</div>

					<div class="field">
						<label>{{ __('BPM') }}</label>
						<input type="text" name="bpm" value="{{ old('bpm', $product->bpm ?? null) }}">
					</div>

					<div class="field">
						<label>{{ __('Bit Rate') }}</label>
						<input type="number" name="bit_rate" value="{{ old('bit_rate', $product->bit_rate ?? null) }}">
					</div>

					<div class="field">
						<label>{{ __('Pages') }}</label>
						<input type="number" name="pages" value="{{ old('pages', $product->pages ?? null) }}">
					</div>

					<div class="field">
						<label>{{ __('Language') }}</label>
						<input type="text" name="language" value="{{ old('language', $product->language ?? null) }}">
					</div>

					<div class="field">
						<label>{{ __('Formats') }}</label>
						<input type="text" name="formats" value="{{ old('formats', $product->formats ?? null) }}">
					</div>

					<div class="field">
						<label>{{ __('Words') }}</label>
						<input type="text" name="words" value="{{ old('words', $product->words ?? null) }}">
					</div>

					<div class="field">
						<label>{{ __('Tools used') }} <i class="exclamation circle icon" title="languages, libraries, frameworks..."></i></label>
						<input type="text" name="software" value="{{ old('software', $product->software ?? null) }}" placeholder="...">
					</div>
					
					<div class="field">
						<label>{{ __('Database used') }} <i class="exclamation circle icon" title="MongoDB, MySQL, SQLite..."></i></label>
						<input type="text" name="database" value="{{ old('database', $product->database ?? null) }}" placeholder="...">
					</div>
					...
					<div class="field">
						...
						<input type="text" name="compatible_browsers" value="{{ old('compatible_browsers', $product->compatible_browsers ?? null) }}" placeholder="...">
					</div>
					...
					<div class="field">
						...
						<input type="text" name="compatible_os" value="{{ old('compatible_os', $product->compatible_os ?? null) }}" placeholder="...">
					</div>
						
					<div class="field">
						<label>{{ __('High resolution') }}</label>
						<div class="ui selection floating dropdown">
							<input type="hidden" name="high_resolution" value="{{ old('high_resolution', $product->high_resolution ?? '0') }}">
							<div class="text">...</div>
							<div class="menu">
								<a class="item" data-value="">-</a>
								<a class="item" data-value="1">{{ __('Yes') }}</a>
					...
					<div class="field">
						...
						<input type="text" name="authors" value="{{ old('authors', $product->authors ?? null) }}">
					</div>

					@if(old('_name_', $product->_name_ ?? null) && old('_value_', $product->_value_ ?? null))
						@foreach(old('_name_', $product->_name_ ?? []) ?? [] as $k => $na)
							<div class="ui segment">
								<div class="two fields">
									...
									<div class="thirteen columns wide field">
										...
										<input type="text" name="_value_[{{ $k }}]" class="addtional-info-value" placeholder="..." value="{{ old('value')[$k] ?? $product->_value_[$k] ?? '' }}">
									</div>
								</div>
								<div class="actions right aligned">
									<i class="times grey circle big icon link" data-action="remove" title="{{ __('Remove') }}"></i>
				...
				<div class="ui tab segment" data-tab="reviews">
					<table class="ui celled unstackable single line table" data-dict='{"Add": "{{ __('Add') }}", "Remove": "{{ __('Remove') }}", "reset": "{{ __('Reset') }}"}'>
						...
						<tbody>
							...
							@endforeach
							@elseif($product->fake_reviews ?? [])
							@foreach($product->fake_reviews as $k => $review)
							<tr>
								<td>
									<input type="text" class="reviews-username" name="fake_reviews[username][{{ $k }}]" value="{{ $review['username'] ?? null }}">
								</td>
								<td class="one colum wide">
									<input type="datetime-local" class="reviews-created_at" name="fake_reviews[created_at][{{ $k }}]" value="{{ format_date($review['created_at'], 'Y-m-d\TH:i') }}">
								</td>
								<td><textarea class="reviews-review" name="fake_reviews[review][{{ $k }}]" rows="2">{{ $review['review'] ?? null }}</textarea></td>
								<td class="one colum wide center aligned">
									<input type="hidden" class="reviews-rating" name="fake_reviews[rating][{{ $k }}]" value="{{ $review['rating'] ?? '0' }}">
									<div class="ui star review large huge rating" data-max-rating="5" data-rating="{{ $review['rating'] ?? '0' }}"></div>
								</td>
								<td class="one column wide center aligned actions">
									<div class="mb-1-hf">
										<button type="button" class="ui small button mr-0 circular icon fluid" data-action="reset">
				...
				<div class="ui tab segment" data-tab="comments">
					<table class="ui celled unstackable single line table" data-dict='{"Add": "{{ __('Add') }}", "Remove": "{{ __('Remove') }}", "reset": "{{ __('Reset') }}"}'>
						...
						<tbody>
							...
							@endforeach
							@elseif($product->fake_comments ?? [])
							@foreach($product->fake_comments as $k => $comment)
							<tr>
								<td>
									<input type="text" class="comments-username" name="fake_comments[username][{{ $k }}]" value="{{ $comment['username'] ?? null }}">
								</td>
								<td class="one colum wide">
									<input type="datetime-local" class="comments-created_at" name="fake_comments[created_at][{{ $k }}]" value="{{ format_date($comment['created_at'], 'Y-m-d\TH:i') }}">
								</td>
								<td><textarea class="comments-comment" name="fake_comments[comment][{{ $k }}]" rows="2">{{ $comment['comment'] ?? null }}</textarea></td>
								<td class="one column wide center aligned actions">
									<div class="mb-1">
										<button type="button" class="ui small button mr-0 circular icon fluid" data-action="reset">
											{{ __('Reset') }}
...
<script type="application/javascript">
	...
	var app = new Vue({
  	...
  	data: {
  		...
  		selectedDrive: '{{ old('file_host', $product->file_host ?? 'local') }}',
  		...
  		searchFile: null,
  		parentFolder: null,
  		fileId: '{!! old('file_name', $product->file_host !== 'local' ? $product->file_name : null) !!}',
  		fileExtension: '{!! old('file_extension', $product->file_extension) !!}',
  		previewExtension: '{!! old('preview_extension', $product->preview_extension) !!}',
  		localFileName: '',
  		...
  		oldUploads: {
  			download: "{{ $download }}", 
  			cover: "{{ $cover }}", 
  			screenshots: "{{ $screenshots ?? ($screenshots_files ? implode(',', $screenshots_files) : '') }}",
  			preview: "{{ $preview }}"
  		}
  	},
  	methods: {
	...
	$(function()
  {
  	...
  	@if(config('app.products_by_country_city'))
	  	...
	  	@if($country = old('country_city.country', $product->country_city->country ?? null))
			$('.ui.dropdown.cities').dropdown({
			...
			@if($city = old('country_city.city', $product->country_city->city ?? null))
	  ...
	  .tab({
		...
		{
			setSubcategories({{ old('category', $product->category) }}, '{{ old('subcategories', $product->subcategories) }}');
		}
		@endif
